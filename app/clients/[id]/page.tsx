      ...
      <header style={{backgroundColor:"#111827",borderBottom:"1px solid #1C253B",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 32px",display:"flex",alignItems:"center",justifyContent:"space-between",height:60}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <Link href="/" style={{display:"flex",alignItems:"center",gap:10,color:"#475569",fontSize:13,transition:"color 0.15s"}}
              ...
              {client.transactions.map((tx: any, i: number) => {
                ...
                return (
                  <div key={tx.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",borderBottom:i<client.transactions.length-1?"1px solid rgba(51,65,85,0.4)":"none",backgroundColor:warn?"rgba(248,113,113,0.04)":"transparent"}}>
                    <div style={{display:"flex",alignItems:"center",gap:12}}>
                      <div style={{width:36,height:36,borderRadius:6,backgroundColor:warn?"rgba(248,113,113,0.12)":"rgba(99,102,241,0.1)",border:`1px solid \${warn?"rgba(248,113,113,0.25)":"rgba(99,102,241,0.2)"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>{warn?String.fromCharCode(9888):"$"}</div>
                      <div>
            ...
            {status === "pending" ? (
              ...
              <div style={{backgroundColor:"rgba(52,211,153,0.07)",border:"1px solid rgba(52,211,153,0.3)",borderRadius:10,padding:"48px 40px",textAlign:"center",boxShadow:"0 0 24px rgba(52,211,153,0.15), 0 8px 32px rgba(0,0,0,0.6)"}}>
                <div style={{width:64,height:64,borderRadius:32,background:"rgba(52,211,153,0.12)",border:"1px solid rgba(52,211,153,0.3)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}}>
                  <span style={{fontSize:28,color:"#34D399"}}>{String.fromCharCode(10003)}</span>
                </div>
                <h2 style={{fontSize:20,fontWeight:700,color:"#34D399",margin:"0 0 10px"}}>Email Sent Successfully</h2><p style={{fontSize:13,color:"#475569",margin:"0 0 24px",lineHeight:1.6}}>The outreach email for {client.name} has been approved and sent to the client. This action is logged for compliance records.</p>
                <Link href="/" style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 24px",borderRadius:6,fontSize:13,fontWeight:600,color:"#34D399",backgroundColor:"transparent",border:"1px solid rgba(52,211,153,0.3)",cursor:"pointer"}}>Return to Dashboard</Link>
              </div>
            ) : rejected ? (
              <div style={{backgroundColor:"rgba(148,163,184,0.07)",border:"1px solid rgba(148,163,184,0.25)",borderRadius:10,padding:"48px 40px",textAlign:"center",boxShadow:"0 8px 32px rgba(0,0,0,0.6)"}}>
                <div style={{width:64,height:64,borderRadius:32,background:"rgba(148,163,184,0.1)",border:"1px solid rgba(148,163,184,0.25)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}}>
                  <span style={{fontSize:28,color:"#94A3B8"}}>{String.fromCharCode(10007)}</span>
                </div>
                ...
                <Link href="/" style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 24px",borderRadius:6,fontSize:13,fontWeight:600,color:"#94A3B8",backgroundColor:"transparent",border:"1px solid #334155",cursor:"pointer"}}>Return to Dashboard</Link>
              </div>
            ) : null}
          </div>
        </div>
